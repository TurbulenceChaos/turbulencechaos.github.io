...
\begin{axiom}
  \begin{enumerate}
  \item If $A$ is a set, then $A$ is also an object.
  \item $A=B$ iff every element of $A$ belongs also to $B$ and vice versa.
  \item There exists an empty set $\emptyset$, such that every object $x \notin \emptyset$.
  \item Let $A$ be a non-empty set. Then there exist an object $x \in \emptyset$.
  \item Singleton set $\{a\}$: $y \in \{a\}$ iff $y=a$.
    Pair set $\{a,b\}$: $y \in \{a,b\}$ iff $y=a$ or $y=b$.
  \item $x \in A \cup B \Leftrightarrow x \in A$ or $x \in B$.
  \end{enumerate}
\end{axiom}
